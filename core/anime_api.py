...
from core.config import HEADERS, ANILIST_API, ANIME_SOURCE_BASE_URL
...
async def search_anime(query: str) -> Optional[List[Dict[str, Any]]]:
    search_url = f"{ANIME_SOURCE_BASE_URL}/api?m=search&q={quote(query)}"
...
async def get_episode_list(session_id: str, page: int = 1) -> Dict[str, Any]:
    episodes_url = f"{ANIME_SOURCE_BASE_URL}/api?m=release&id={session_id}&sort=episode_asc&page={page}"
...
def get_latest_releases(page=1):
    releases_url = f"{ANIME_SOURCE_BASE_URL}/api?m=airing&page={page}"
...
def get_stream_links(anime_session: str, episode_session: str) -> Optional[List[Dict[str, Any]]]:
    if '-' in episode_session:
        episode_url = f"{ANIME_SOURCE_BASE_URL}/play/{episode_session}"
    else:
        episode_url = f"{ANIME_SOURCE_BASE_URL}/play/{anime_session}/{episode_session}"
    ...
    try:
        ...
        session.get(f"{ANIME_SOURCE_BASE_URL}/")
...
def extract_m3u8_from_kwik(kwik_url: str) -> Optional[Dict[str, Any]]:
    try:
        ...
        animepahe_referer = f"{ANIME_SOURCE_BASE_URL}/"
