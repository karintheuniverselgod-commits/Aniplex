...
def get_admins_from_env():
    raw = os.environ.get("ADMINS") or os.environ.get("ADMIN_CHAT_ID")

    if not raw:
        raise ValueError(
            "ADMINS or ADMIN_CHAT_ID must contain at least one Telegram user ID"
        )
...
DUMP_CHANNEL_ID = get_env_var(
    "DUMP_CHANNEL_ID",
    required=False,
)

DUMP_CHANNEL_USERNAME = get_env_var(
...
if CHANNEL_ID:
    try:
        ...
        logger.warning(
            "Invalid CHANNEL_ID provided - must be a number. "
            "Falling back to username if available."
        )
        CHANNEL_ID = None
...
if CHANNEL_USERNAME:
    ...
    logger.info(
        f"Channel username configured: {CHANNEL_USERNAME}"
    )
...
if DUMP_CHANNEL_ID:
    try:
        ...
        logger.info(
            f"Dump Channel ID configured: {DUMP_CHANNEL_ID}"
        )

    except ValueError:
        logger.warning(
            "Invalid DUMP_CHANNEL_ID provided - must be a number. "
            "Falling back to username if available."
        )
...
if DUMP_CHANNEL_USERNAME:
    ...
    logger.info(
        f"Dump channel username configured: {DUMP_CHANNEL_USERNAME}"
    )


if not CHANNEL_ID and not CHANNEL_USERNAME:
    logger.warning(
        "No main channel ID or username configured. "
        "Files will only be sent to dump channel."
    )


if not DUMP_CHANNEL_ID and not DUMP_CHANNEL_USERNAME:
    logger.warning(
        "No dump channel ID or username configured. "
        "Files will only be sent to users directly."
    )
...
START_PIC_URL = get_env_var(
    "START_PIC_URL",
    "https://imgs.search.brave.com/9n5_FAipMAH3ic1LtbHx3btylHNWppO2rl4gXnRjr1g/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly93YWxs/cGFwZXJjYXZlLmNvbS93cC93cDE4MzM1NTIuanBn",
    required=False,
)
...
SUPPORT_CHANNEL_URL = get_env_var(
    "SUPPORT_CHANNEL_URL",
    "",
    required=False,
)
...
ANIME_SOURCE_BASE_URL = get_env_var(
...
HEADERS = {
    "authority": ANIME_SOURCE_BASE_URL.split("://", 1)[-1],
    ...
    "referer": f"{ANIME_SOURCE_BASE_URL}/",
    "user-agent": (
...
<code>/cancel</code> - ᴄᴀɴᴄᴇʟ ᴄᴜʀʀᴇɴᴛ ᴏᴘᴇʀᴀᴛɪᴏɴ
...
<code>/set_request_time [HH:MM]</code> - sᴇᴛ ᴅᴀɪʟʏ ʀᴇǫᴜᴇsᴛ ᴘʀᴏᴄᴇssɪɴɢ ᴛɪᴍᴇ
<code>/set_max_requests [number]</code> - sᴇᴛ ᴍᴀxɪᴍᴜᴍ ɴᴜᴍʙᴇʀ ᴏғ ᴄᴏɴᴄᴜʀʀᴇɴᴛ ʀᴇǫᴜᴇsᴛs
<code>/view_requests</code> - ᴠɪᴇᴡ ᴘᴇɴᴅɪɴɢ ʀᴇǫᴜᴇsᴛs
<code>/set_request_group [group_id]</code> - sᴇᴛ ᴛʜᴇ ʀᴇǫᴜᴇsᴛ ɢʀᴏᴜᴘ
<code>/request [anime name]</code> - ʀᴇǫᴜᴇsᴛ ᴀɴ ᴀɴɪᴍᴇ
<code>/addtask [number]</code> - ᴅᴏᴡɴʟᴏᴀᴅ sᴘᴇᴄɪғɪᴄ ᴀɴɪᴍᴇ
<code>/redownload [number]</code> - ғᴏʀᴄᴇ ʀᴇᴅᴏᴡɴʟᴏᴀᴅ ᴀ sᴘᴇᴄɪғɪᴄ ᴀɴɪᴍᴇ
<code>/add_admin [user_id]</code> - ᴀᴅᴅ ᴀ ɴᴇᴡ ᴀᴅᴍɪɴ
<code>/remove_admin [user_id]</code> - ʀᴇᴍᴏᴠᴇ ᴀɴ ᴀᴅᴍɪɴ
</blockquote>
──────────────────
<blockquote>≡ ᴘᴏᴡᴇʀᴇᴅ ʙʏ: {BOT_DISPLAY_NAME}</blockquote>
</b>"""
